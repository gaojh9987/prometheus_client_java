...
public class Exporter {
    ...
    public void doGet(final HttpServletRequestAdapter req, final HttpServletResponseAdapter resp) throws IOException {
        ...
        try {
            ...
            if (filter == null) {
                TextFormat.writeFormat(contentType, writer, registry.metricFamilySamples(req.getParameterValue("reset") != null));
            } else {
                TextFormat.writeFormat(contentType, writer, registry.filteredMetricFamilySamples(filter, req.getParameterValue("reset") != null));
            }
            writer.flush();
        } finally {
            writer.close();
